...
//				$('tr[class ="datagrid-header-row"]').find('td[field="cuozuo"]').hide();
		...
		onClickCell:function(index,field,value){
			...
			if(field == 'cuozuo'){
				...
				if (row){
					if(row.SJRQ != "暂无数据"){
						$('#dlg').dialog('open').dialog('setTitle','编辑 金融机构信息');
						$('#insertData').attr("style","display:none;");
						$('#saveData').attr("style","display:block;");
						$('#saveData').attr("style","width:90px;");
						// 编辑时内部机构号不可编辑
						$("input[name='NBJGH'][type='hidden'].textbox-value").prev().attr("readonly","readonly");
						$('#fm').form('load',row);
					}
				}
			}else{
	...
	pager.pagination({  
	    ...
	    buttons:[{
			...
			handler:function(){
				...
				// 新增时内部机构号可编辑
				$("input[name='NBJGH'][type='hidden'].textbox-value").prev().removeAttr("readonly");
				$('#fm').form('load',[]);
				$("#fm").form("reset");  
			}
		}] 
...
//编辑保存
function saveData(){
	$('#UpdateSqlId').val("finfUpdateData");
	$('#fm').form('submit', {  
