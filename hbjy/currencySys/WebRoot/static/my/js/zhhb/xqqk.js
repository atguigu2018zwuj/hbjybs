...
function doSearch(grid){
	...
	var data = 
	{
		...
		DYLX :   $('#DYLX').val(),
		QB : $('#QB').val(),
		XJXQ_JQ_GS: $('#XJXQ_JQ_GS').val(),
		XJXQ_YB_GS :$('#XJXQ_YB_GS').val(),
		XJXQ_CF_GS :$('#XJXQ_CF_GS').val(),
		SBLX :   $('#SBLX').val(),
...
function chaxun(index,grid,data){
	...
	grid.datagrid({
		url:url+"zhhbController/getCodeData",
		queryParams:data,
		fitColumns: false,
		nowrap:true,
		pagination:true,
					...
					if (r){
						...
						if (delData.length <= 0){
							...
							var d_da = {
								...
								delSqlId:"xqqkDeleteData",
								fileName:$("#fileName").val()
							}
							$.ajax({
								type:"post",
								url:url+"zhhbController/deleteData",
								data:d_da,
...
function saveData(){
	...
	$('#UpdateSqlId').val("xqqkUpdateData");
	$('#fm').form('submit', {  
        url:url+'zhhbController/updateData',  
        onSubmit: function(){  
                if($("#fm").form("validate")) {
                    return true  
                }else {	
