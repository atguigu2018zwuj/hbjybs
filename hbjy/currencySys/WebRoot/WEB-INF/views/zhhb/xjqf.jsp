	...
	<div class="consult common" id="sssss">
		...
		<input type="hidden" value="<%=basePath %>" id="url">
					...
					<form id="formSub" <c:if test="${provincialUsers !=  nbjgh}">style="height: 43px;"</c:if> <c:if test="${provincialUsers ==  nbjgh}">style="height: 88px;"</c:if>  >
						<ul>
						  ...
						  <c:if test="${provincialUsers ==  nbjgh}">
							...
							<li><span><input type="hidden" id="excelSqlid" name="excelSqlid" value="getXjqfData"/></span></li>
						</ul>
					</form>
				...
				<div class="consult-right" style="margin-right: -5px;">
					<div class="consult-history" style="padding-bottom: 50px;">
						<div class="consult-head">
							<span class="consult-title">搜索结果-银行业金融机构现金清分情况统计表		(增量表-季报)</span>
						</div>
						<div id="myTable" style="display:none;">
							<!-- 引入通用页面 -->
				            <jsp:include page="../currencyJsp/content.jsp">
				            	<jsp:param value="xjqf" name="str"/>
				            	<jsp:param value="SYYH_XJQF" name="tablename"/>
				            </jsp:include>
						</div>
					</div>
				</div>
			...
			<script type="text/javascript">
		    	seajs.use('myJsPath/js/zhhb/xjqf');
		    </script>
		</div>
</body>
</html>