...
define(function(require) {
	...
	$(function() {
		...
		$('#more').click(function(){
			...
			$('ul:eq(0)').attr("style","height:178px;");
		});
		...
		if (searchParams != undefined && Object.keys(searchParams).length > 0) {
			...
			for (var key in keys) {
				$("#formSub").find("[name='"+keys[key]+"']").val(searchParams[keys[key]]);
			}
			$('#more').click();
			doSearch(grid);
		}
...
function doSearch(grid){
	...
	var data = 
	{
		...
		JGJB : $("#JGJB").val(),
		SJGLJGMC : $("#SJGLJGMC").val(),
		SJGLJGJRJGBM : $("#SJGLJGJRJGBM").val(),
		SJGLJGNBJGH : $("#SJGLJGNBJGH").val(),
		JGLB : $("#JGLB").val(),
		DQDM : $("#DQDM").val(),
		CLSJ : $("#CLSJ").val(),
		YYZT : $("#YYZT").val(),
...
function chaxun(index,grid,data){
	...
	grid.datagrid({
		url:"/currencySys/manaProController/getCodeData",
		queryParams:data,
		fitColumns: false,
		nowrap:true,
		pagination:true,
			...
			if(field == 'cuozuo'){
				...
				if (row){
					if(row.SJRQ != "暂无数据"){
						...
						// 编辑时内部机构号不可编辑
						$("input[name='BR_NO'][type='hidden'].textbox-value").prev().attr("readonly","readonly");
						$('#fm').form('load',row);
					}
				}
			}else{
	...
	pager.pagination({  
	    ...
	    buttons:[{
					...
					if (r){
						...
						if (delData.length <= 0){
							...
							var d_da = {
								...
								delSqlId:"jrjgxxDeleteData",
								fileName:$("#fileName").val()
							}
							$.ajax({
								type:"post",
								url:"/currencySys/manaProController/deleteData",
								data:d_da,
			...
			handler:function(){
				...
				// 新增时内部机构号可编辑
				$("input[name='BR_NO'][type='hidden'].textbox-value").prev().removeAttr("readonly");
				$('#fm').form('load',[]);
				$("#fm").form("reset");
			}
		}]  
...
function download(grid){
	$dialog.confirm({
        ...
        ok: function(){
        	...
        	//window.location.href = url+"manaProController/jgxxDown?"+param+"&fields="+encodeURIComponent(JSON.stringify(fields))+"&fileName=jgxx&beginSjrq="+formatDate(new Date())+"&fileTitle="+encodeURIComponent(title)+"&fieldValues="+encodeURIComponent(JSON.stringify(fieldValues));
        	window.location.href = url+"manaProController/jgxxDown?"+param+"&fields="+encodeURIComponent(JSON.stringify(fields))+"&fileName="+fileName+"&fileTitle="+encodeURIComponent(title)+"&fieldValues="+encodeURIComponent(JSON.stringify(fieldValues));
        },
        cancelValue: '取消',
            cancel: true,
            onclose: function(){}
...
//编辑保存
function saveData(){
	$('#UpdateSqlId').val("jrjgxxUpdateData");
	$('#fm').form('submit', {  
