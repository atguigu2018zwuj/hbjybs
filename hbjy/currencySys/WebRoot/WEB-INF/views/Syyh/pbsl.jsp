...
<html lang="en">
<head>
    <title>硬币自循环情况-硬币自助设备配备数量</title>
		...
		<input type="hidden" value="<%=basePath %>" id="url">
			<div class="searchMainBox" style="height: auto;">
				...
				<div class="accurateSearchInputs">
					<form id="formSub" <c:if test="${provincialUsers !=  nbjgh}">style="height: 43px;"</c:if> <c:if test="${provincialUsers ==  nbjgh}">style="height: 88px;"</c:if>  >
						<ul>
							...
							<li><span>硬币自助设备类型</span><input type="text" id="YBSBLX" name="YBSBLX" placeholder="支持模糊查询"/></li>
							...
							<li><span><input type="hidden" id="excelSqlid" name="excelSqlid" value="getPbslData"/></span></li>
						</ul>
					</form>
				...
				<div class="consult-right" style="margin-right: -5px;">
					<div class="consult-history" style="padding-bottom: 50px;">
						<div class="consult-head">
							<span class="consult-title">搜索结果-硬币自循环情况-硬币自助设备配备数量		(全量表-季报)</span>
						</div>
						<div class="" id="myTable" style="display:none;">
							<!-- 引入通用页面 -->
				            <jsp:include page="../currencyJsp/content.jsp">
				            	<jsp:param value="pbsl" name="str"/>
				            	<jsp:param value="SYYH_PBSL" name="tablename"/>
				            </jsp:include>
						</div>
					</div>
				</div>
			...
			<script type="text/javascript">
		    	seajs.use('myJsPath/js/Syyh/pbsl');
		    </script>
		</div>
</body>
</html>