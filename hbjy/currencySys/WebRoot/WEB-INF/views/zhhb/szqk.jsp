	...
	<div class="consult common" id="sssss">
		...
		<input type="hidden" value="<%=basePath %>" id="url">
				...
				<div class="accurateSearchInputs">
					<form id="formSub" <c:if test="${provincialUsers !=  nbjgh}">style="height: 43px;"</c:if> <c:if test="${provincialUsers ==  nbjgh}">style="height: 88px;"</c:if>  >
						<ul>
							...
							<li><span>项目名称</span><input type="text" id="XMMC" name="XMMC" placeholder="支持模糊查询"/></li>						   					   
							...
							<li><span><input type="hidden" id="excelSqlid" name="excelSqlid" value="getSzqkData"/></span></li>
						</ul>
					</form>
				...
				<div class="consult-right" style="margin-right: -5px;">
					<div class="consult-history" style="padding-bottom: 50px;">
						<div class="consult-head">
							<span class="consult-title">搜索结果-银行业金融机构现金收支情况统计表		(增量表-季报)</span>
						</div>
						<div id="myTable" style="display:none;">
							<!-- 引入通用页面 -->
				            <jsp:include page="../currencyJsp/content.jsp">
				            	<jsp:param value="szqk" name="str"/>
				            	<jsp:param value="SYYH_SZQK" name="tablename"/>
				            </jsp:include>
						</div>
					</div>
				</div>
			...
			<script type="text/javascript">
		    	seajs.use('myJsPath/js/zhhb/szqk');
		    </script>
		</div>
</body>
</html>