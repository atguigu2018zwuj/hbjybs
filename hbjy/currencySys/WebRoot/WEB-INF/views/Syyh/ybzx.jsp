	...
	<div class="consult common" id="sssss">
		...
		<input type="hidden" value="<%=basePath %>" id="url">
				...
				<div class="accurateSearchInputs">
					<form id="formSub" <c:if test="${provincialUsers !=  nbjgh}">style="height: 43px;"</c:if> <c:if test="${provincialUsers ==  nbjgh}">style="height: 88px;"</c:if>  >
						<ul>
							...
							<li><span>券别</span><input type="text" id="QB" name="QB" placeholder="支持模糊查询"/></li>
							...
							<li><span><input type="hidden" id="excelSqlid" name="excelSqlid" value="getYbzxData"/></span></li>
						</ul>
					</form>
				...
				<div class="consult-right" style="margin-right: -5px;">
					<div class="consult-history" style="padding-bottom: 50px;">
						<div class="consult-head">
							<span class="consult-title">搜索结果-硬币自循环情况-硬币自循环数量   (增量表-季报)</span>
						</div>
						<div class="" id="myTable" style="display:none;">
							<!-- 引入通用页面 -->
				            <jsp:include page="../currencyJsp/content.jsp">
				            	<jsp:param value="ybzx" name="str"/>
				            	<jsp:param value="SYYH_YBZX" name="tablename"/>
				            </jsp:include>
						</div>
					</div>
				</div>
			...
			<script type="text/javascript">
		    	seajs.use('myJsPath/js/Syyh/ybzx');
		    </script>
		</div>
</body>
</html>