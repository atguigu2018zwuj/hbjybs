...
function doSearch(grid){
	...
	var data = 
	{
		...
		NBJGH : $('#NBJGH').val(),
		SZQY : $('#SZQY').val(),
		XJSR_GM_BQ : $('#XJSR_GM_BQ').val(),
		XJSR_GM_NLJ : $('#XJSR_GM_NLJ').val(),
		XJSR_ZZ_BQ : $('#XJSR_ZZ_BQ').val(),
		XJSR_ZZ_NLJ : $('#XJSR_ZZ_NLJ').val(),
		XJZC_GM_BQ : $('#XJZC_GM_BQ').val(),
		XJZC_GM_NLJ : $('#XJZC_GM_NLJ').val(),
		XJZC_ZZ_BQ : $('#XJZC_ZZ_BQ').val(),
		XJZC_ZZ_NLJ : $('#XJZC_ZZ_NLJ').val(),
		CSBDH_BS_BQ : $('#CSBDH_BS_BQ').val(),
		CSBDH_BS_NLJ : $('#CSBDH_BS_NLJ').val(),
		CSBDH_JE_BQ : $('#CSBDH_JE_BQ').val(),
		CSBDH_JE_NLJ : $('#CSBDH_JE_NLJ').val(),
		QBTJ_BS_BQ : $('#QBTJ_BS_BQ').val(),
		QBTJ_BS_NLJ : $('#QBTJ_BS_NLJ').val(),
		QBTJ_JE_BQ : $('#QBTJ_JE_BQ').val(),
		SBLX : $('#SBLX').val(),
...
function chaxun(index,grid,data){
	...
	grid.datagrid({
		url:"/currencySys/applePlusController/getCodeData",
		queryParams:data,
		fitColumns: false,
		nowrap:true,
		pagination:true,
					...
					if (r){
						...
						if (delData.length <= 0){
							...
							var d_da = {
								...
								delSqlId:"rbsfDeleteData",
								fileName:$("#fileName").val()
							}
							$.ajax({
								type:"post",
								url:"/currencySys/applePlusController/deleteData",
								data:d_da,
...
function saveData(){
	...
	$('#UpdateSqlId').val("rbsfUpdateData");
	$('#fm').form('submit', {  
        url:'/currencySys/applePlusController/updateData',  
        onSubmit: function(){  
                if($("#fm").form("validate")) {
                    return true  
                }else {	
