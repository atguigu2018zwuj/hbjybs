...
function chaxun(index,grid,data){
	...
	grid.datagrid({
		url:url+"jrjgController/getCodeData",
		queryParams:data,
		fitColumns: false,
		nowrap:true,
		pagination:true,
					...
					if (r){
						...
						if (delData.length <= 0){
							...
							var d_da = {
								...
								delSqlId:"jrszDeleteData"
							}
							$.ajax({
								type:"post",
								url:url+"jrjgController/deleteData",
								data:d_da,
...
//编辑保存
function saveData(){
	$('#UpdateSqlId').val("JrszUpdateData");
	$('#fm').form('submit', {  
        url:url+'jrjgController/updateData',  
        onSubmit: function(){  
                if($("#fm").form("validate")) {
                    return true  
                }else {	
